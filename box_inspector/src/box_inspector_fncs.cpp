...
bool BoxInspector::model_poses_wrt_box(osrf_gear::Shipment &shipment_status) {
    ...
    geometry_msgs::PoseStamped box_pose_wrt_world, part_pose_wrt_world, cam_pose_wrt_box;
    Eigen::Affine3d affine_cam_wrt_world, affine_part_pose_wrt_cam, affine_part_pose_wrt_box,
            affine_box_pose_wrt_world, affine_part_pose_wrt_world, affine_cam_pose_wrt_box, affine_box_pose_wrt_cam;
    ...
    for (int imodel = 0; imodel < num_models; imodel++) {
        if (imodel != i_box) { //if here, have a model NOT the box
            ...
            affine_cam_pose_wrt_box= affine_box_pose_wrt_cam.inverse();
            cam_pose_wrt_box=xformUtils_.transformEigenAffine3dToPoseStamped(affine_cam_pose_wrt_box);
            affine_part_pose_wrt_cam= xformUtils_.transformPoseToEigenAffine3d(model_pose_wrt_cam);
            affine_part_pose_wrt_box= affine_cam_pose_wrt_box * affine_part_pose_wrt_cam;
            part_pose_wrt_box= xformUtils_.transformEigenAffine3dToPose(affine_part_pose_wrt_box);
            //part_pose_wrt_box= compute_stPose(cam_pose_wrt_box, model_pose_wrt_cam);
...
void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment shipment_wrt_box,
	   ...
	   Eigen::Affine3d affine_shipment_pose_wrt_box = xformUtils_.transformPoseToEigenAffine3d(shipment_wrt_box.products[i].pose);
	   Eigen::Affine3d affine_box_pose_wrt_world = xformUtils_.transformPoseToEigenAffine3d(box_pose_wrt_world.pose);
	   Eigen::Affine3d affine_shipment_pose_wrt_world = affine_box_pose_wrt_world*affine_shipment_pose_wrt_box;	
           geometry_msgs::Pose shipment_pose_wrt_world = xformUtils_.transformEigenAffine3dToPose(affine_shipment_pose_wrt_world);
           model.type=shipment_wrt_box.products[i].type;
	   model.pose=shipment_pose_wrt_world;
