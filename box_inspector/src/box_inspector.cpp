...
// orphan_models_wrt_world: vector of models that are seen in the box, but DO NOT belong in the box
  ...
  for (int i=0;i< num_parts_seen;i++) {
   ...
   bool checkfororphan = false;
   for (int j = 0; j < num_parts_desired; j++) {
    model_desired = desired_models_wrt_world[j];
    float dx,dy,dz,translation_error, rotational_error;
    ...
    translation_error = sqrt(dx*dx + dy*dy + dz*dz);          
     float rotation_seen = xformUtils_.convertPlanarQuat2Phi(model_seen.pose.orientation);
     float rotation_desired = xformUtils_.convertPlanarQuat2Phi(model_desired.pose.orientation);
              
    rotational_error = rotation_seen - rotation_desired;
    if (rotational_error < 0) {
    rotational_error = -1*rotational_error;
    }
    if (model_seen.type == model_desired.type && translation_error<=0.03 && rotational_error<=0.1){
    satisfied_models_wrt_world.push_back(model_seen);
    checkfororphan=true;
    break;
    }
    if (model_seen.type == model_desired.type && (translation_error>0.03 || rotational_error>0.1)){
              misplaced_models_actual_coords_wrt_world.push_back(model_seen);
  ...
  if(!checkfororphan){
  orphan_models_wrt_world.push_back(model_seen);
  }
 }
 for (int j = 0; j < num_parts_desired; j++) {
 model_desired = desired_models_wrt_world[j];
 bool checkformissing= false;
 for (int i=0;i<num_parts_seen;i++) {
  model_seen = models_seen_wrt_world[i];
  if (model_desired.type ==model_seen.type){
